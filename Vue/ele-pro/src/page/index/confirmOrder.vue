<template>
  <div class="confirmOrder flex_column fixed_full bg_gray">
    <Top :nav_title="nav_title"
    ...
    <div class="confirmOrder_content common_container">
      <div class="confirmOrder_content_top bg_yellow rel flex_center">
        <div
          ...
          @click.stop="toLoginOrMyAddress(loginFlag)"
        >
          <div
          ...
          >
            <div v-if="loginFlag == 2">
              <div class="weight_600 ellipsis detail_address font15">
                {{ defaultAdd.detail_address }}
              </div>
      ...
      <div class="confirmOrder_container bg_gray rel">
        <div class="order_detail bg_white border_radius_5 rel padding_10 flex_col">
          ...
          <div class="unfold flex_center" v-show="1 == 0">
        ...
        <div
          class="order_detail order_detail_btm bg_white border_radius_5 rel padding_10 flex_col margin_top_10"
        >
          <div
    ...
    >
      <div class="commit_order_price bg_black rel flex_center">
        ...
        <span class="span_white font16 weight_600 abs" style="padding-left: 10px"
        >￥{{ foodsPrice }}</span
        >
      </div>
...
export default {
  ...
  watch: {},
  methods: {
    toLoginOrMyAddress(loginFlag) {
      // 未登录
      const guest = 0;
      this.$router.replace({
        path: guest == loginFlag ? "/login" : "/myAddress",
      });
    },
    //TODO 补充没有地址的情况
    getAddress() {
    ...
    info() {
      //是否登录
      this.userInfo = getStore("loginUserInfo") && JSON.parse(getStore("loginUserInfo"));
      // 用户是否有地址
      ...
      if (this.type != "order") {
        ...
        //来自订单页面
        data = qs.stringify({order_id: this.$route.query.order_id});
        this.$axios.post("/api/getOrderDetail", data).then((res) => {
          that.amountList = JSON.parse(res.data[0].order_detail);
          that.foodsPrice = res.data[0].all_price;
          that.shop_img = res.data[0].shop_img;
          that.restaurant_name = res.data[0].restaurant_name;
        });
      }
    },
    commitOrder() {
