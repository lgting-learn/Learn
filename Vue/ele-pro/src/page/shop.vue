...
TODO
   -->
    ...
    >
      <div class="shop_top flex_row padding_10 grow_0 bg_black_2E2F3B">
        ...
        <img
          ...
          @click="showPreview(shopDetail.preview_index)"
        />
        <div class="shop_top_right flex_column">
          <h3 class="ellipsis span_white font16 padding_5_bottom">
            {{ shopDetail.name }}
          </h3>
      ...
      <!-- 商品tab -->
      <div v-if="active == 0" class="shop_container grow_1">
        <!-- <div
      v-if="active==1"
    ...
    > -->
        ...
        >
          <!-- 左侧菜单 -->
          <div class="goodMenu grow_0" ref="goodMenu">
          ...
          <!-- 右侧菜单 -->
          <div class="goodList grow_1" ref="goodList">
            <ul class="ul_right">
              <li v-for="(items, indexs) in goodList" :key="indexs" ref="rItem">
                ...
                >
                  <van-image
                    ...
                    @click="showPreview(itemss.preview_index)"
                  />
        ...
        <van-popup v-model="show" position="bottom" :style="{ height: '30%' }">
          ...
          <div class="flex_col list_detail">
            ...
            >
              ...
              <div class="addAndReduce addAndReduce_cart">
                ...
                <div class="img_pic_wrap">
                  ...
                  >
                    <van-image
                      ...
                      @click="showPreview(img_pic.preview_index)"
                    />
                  </div>
                </div>
              </div>
  ...
  methods: {
    ...
    initData() {
      ...
      this.$axios.post("/api/shopFoodClassify", data).then(function (res) {
        that.goodListReal = res.data;
        if (that.goodListReal) {
          for (let i = 0; i < that.goodListReal.length; i++) {
      ...
      this.$axios.post("/api/shopSingle", data).then((res) => {
        ...
        that.imgArr.push(that.shopDetail.image_path);
        that.shopDetail.preview_index = that.imgArr.length - 1;
                ...
                for (let j = 0; j < that.appraiseList[i].appraise_img.length; j++) {
                  that.appraiseList[i].appraise_img[j].url =
                    that.appraiseList[i].appraise_img[j].url.indexOf("?uploadId") > -1
                      ? that.appraiseList[i].appraise_img[j].url.split("?uploadId")[0]
                      : that.appraiseList[i].appraise_img[j].url;
    ...
    initCart() {
      ...
      this.$axios.post("/api/shopFoodClassify", data).then(function (res) {
        ...
        let goodListArr = that.goodListCart;
        let itemObj = "";
        if (goodListArr.length != 0) {
          that.getFoodsTotal(that.goodListCart);
        }
      });
    },
    //改变食品选购数量
