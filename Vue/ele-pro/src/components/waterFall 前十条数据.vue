...
export default {
  ...
  methods: {
    ...
    calItemPosition() {
      ...
      for (var j = 0; j < this.imgsArr.length; j++) {
        tempCache[j].src = this.imgsArr[j].src;
        var height = this.imgsArr[j].height; //获取图片高度
        var width = this.imgsArr[j].width; //获取图片宽

        // 压缩比
        let rate = width / itemWidth;
        // 瀑布流图片容器宽度
        this.imgsArr[j].itemW = itemWidth;
        let itemH = height / rate;
        // 瀑布流图片容器高度
        this.imgsArr[j].itemH = itemH;
        // 第一列
        if (j < this.columns) {
          arrH.push(itemH);
          tempCache[j].top = 0;
          // 图片容器宽度 + j份左边距
          tempCache[j].left = itemWidth * j + this.waterFulpadding * j;
        } else {
          // 其他列
          ...
          // 设置下一行的第一个盒子位置
          // top值就是最小列的高度+底部距离
          tempCache[j].top = arrH[minIndex] + this.waterFullBottom;
          // left跟随第一列即可
          tempCache[j].left = tempCache[minIndex].left;

          // 修改最小列的高度
          // 最小列的高度 = 最小列高度 + 自身高度 + 底部距离
          arrH[minIndex] = arrH[minIndex] + itemH + this.waterFullBottom; //设置距离
        }
        tempCache[j].itemH = itemH;
      }
      return tempCache;
    },
          ...
          img.onload = function () {
            if (this.complete) {
              ...
              if (loadFinishNums == that.imgsArr.length) {
                let tempCache = that.calItemPosition();
                that.tempCache = tempCache;
              }
            }
          };
